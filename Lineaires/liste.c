...
10/2025
Implématation de listes chaînées en C
*/
...
// Ici on a le problème original avec la structure récursive Node, donc on crée
// une struct "Liste" pour pouvoir manier les Node
...
void ajouter_tete(Liste *lst, int elt) {
  ...
  // On met la valeur souhaité dans le maillon, puis on met son pointeur sur
  // l'ancien maillon tête de la liste
  new->data = elt;
  new->next = lst->tete;
...
char *str_liste(Liste *lst) {
  ...
  Node *waza = lst->tete; // Au lieu de Node waza = *lst->tete; car ici on
                          // aurait crée une copie de lst->tete au lieu de juste
                          // le manipuler, qui est safe dans ce cas
  Node *current = lst->tete;
  int taille_totale = 4;
  ...
  while (current != NULL) {
    // chaine += waza.data + {'-', '>', ' '}; illégal car {'-', '>', ' '} est
    // pour INITIALISER, de plus,
    // "+=" n'est pas possible en C car il n'y a pas d'overload pour les char*
    // (pythonerie attention) Alors, on fait plus classiquement chaine = chaine
    // + waza.data + "-> " N'est tout de même pas possible car c'est du C...😽
    // On doit utiliser strcat de <string.h>, car la concaténation n'est pas
    // imédiate.
    //
...
/*
int main() {
  // Tests pour s'assurer du bon comportement de la liste
  Liste ma_liste;
  init_liste(&ma_liste);
